...
namespace Graph {
    ...
    public:
        ...
        const std::set<std::string>& Deleted() const {return deleted;}//获得被删除表
        const int NumVertices() const { return n_verts;}//获得顶点数
        const int EdgeVertices() const { return m_edges; }//获得边数
        void InsertVertex(const LocationInfo& vertex_info); // 插入点
        void DeleteVertex(const LocationInfo& vertex_info); // 删除点，同时删除相邻边
        void UpdateVertex(const std::string& name, int new_time); //修改点信息
        VertInfo GetVertex(const std::string& name) const; // 通过地点名 查询点
        VertInfo GetsametypeVertex(const std::string& name) const; // 通过类型名 查找所有点
        ...
        std::vector<EdgeNode> GetEdgesFromLocation(const std::string& location_name) const;//查询点的所有边
